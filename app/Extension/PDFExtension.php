...
class PDFExtension extends \Twig_Extension
{
     ...
     * @return FPDF
     */
    public static function add_page($pdf) {
        $pdf->AddPage();
     ...
     * @return FPDF
     */
    public static function add_qrcode($pdf, $postion, $nomBalise) {
        $nomBaliseUTF8 = utf8_decode($nomBalise);

        switch ($postion) {
            case 1:
                $pdf->Text(35, 12, $nomBaliseUTF8);
                $pdf->Image('./img/qrcode_' . $nomBalise . '.png', 17.5, 17, 80);
                break;
            case 2:
                $pdf->Text(140, 12, $nomBaliseUTF8);
                $pdf->Image('./img/qrcode_' . $nomBalise . '.png', 112.5, 17, 80);
                break;
            case 3:
                $pdf->Text(35, 108, $nomBaliseUTF8);
                $pdf->Image('./img/qrcode_' . $nomBalise . '.png', 17.5, 112, 80);
                break;
            case 4:
                $pdf->Text(140, 108, $nomBaliseUTF8);
                $pdf->Image('./img/qrcode_' . $nomBalise . '.png', 112.5, 112, 80);
                break;
            case 5:
                $pdf->Text(35, 203, $nomBaliseUTF8);
                $pdf->Image('./img/qrcode_' . $nomBalise . '.png', 17.5, 207, 80);
                break;
            case 6:
                $pdf->Text(140, 203, $nomBaliseUTF8);
                $pdf->Image('./img/qrcode_' . $nomBalise . '.png', 112.5, 207, 80);
                break;
        }
    }

    public static function telecharger_pdf($pdf, $nomCourse) {
